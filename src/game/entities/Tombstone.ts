...
export class Tombstone extends Phaser.GameObjects.Graphics {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number) {
    ...
    this.setDepth(5)
    this.setInteractive(
      new Phaser.Geom.Circle(0, 0, AMOEBA_RADIUS_PX * 1.5),
  ...
  private drawShape(): void {
    this.clear()
    // Simple tombstone shape: gray rounded rectangle
    const w = AMOEBA_RADIUS_PX * 1.2
    const h = AMOEBA_RADIUS_PX * 2
    this.fillStyle(0x555555, 0.8)
    this.lineStyle(1, 0x333333, 0.6)
    this.fillRoundedRect(-w / 2, -h / 2, w, h, 4)
    this.strokeRoundedRect(-w / 2, -h / 2, w, h, 4)
  }
}
